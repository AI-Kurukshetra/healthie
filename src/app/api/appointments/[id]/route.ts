...
async function ensureAppointmentAccess(supabase: ReturnType<typeof createSupabaseRouteHandlerClient>, userId: string, role: string, appointment: { patient_id: string; provider_id: string }) {
  if (role === "patient") {
...
async function notifyAppointmentParticipants(
  ...
  const [patientQuery, providerQuery] = await Promise.all([
    ...
    supabase.from("providers").select("user_id").eq("id", appointment.provider_id).single()
  ]);
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const canAccess = await ensureAppointmentAccess(supabase, user.id, profile.role, appointment);
  if (!canAccess) {
  ...
  if (body.scheduled_at) {
    const availabilityQuery = await listProviderAvailability(supabase, appointment.provider_id);
  ...
  const { data, error } = await updateAppointment(supabase, params.id, payload);
  if (error || !data) {
  ...
  await notifyAppointmentParticipants(supabase, data, user.id, "Appointment updated", bodyText);

  await createAuditLog("appointment.updated", "appointments", data.id, payload);
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const appointmentQuery = await supabase.from("appointments").select("patient_id, provider_id").eq("id", params.id).maybeSingle();
  ...
  await notifyAppointmentParticipants(supabase, appointment, user.id, "Appointment cancelled", "An appointment has been cancelled.");
  await createAuditLog("appointment.deleted", "appointments", params.id, {});
  refreshPortalPaths(["/patient/appointments", "/provider/appointments"]);
